    ...
    private static String getRamdonName(){
        ...
        for (int i =0; i < length; i++){
            ...
            do {
                ...
                for (int val : vowels){
                    if (i == 0) isVowelsPrev = true;
                    else if (name.charAt(i-1) == val) isVowelsPrev = true;
                    if ((int)letter == val) isVowelsCur = true;
                }
...
/*if ($sendMessageResponse1/response/responseInfo/status/code = SCSS001) then (
    		if (fn:contains($sendMessageResponse1/response/responseData/data, "&lt;")) then (
        fn-bea:inlinedXML(fn:replace($sendMessageResponse1/response/responseData/data), "&lt;", "<")
        ) else ($sendMessageResponse1/response/responseData/data)
        ) else ("<errorCode>500</errorCode>
    <errorMessage>Internal Server Error/Внутренняя ошибка сервера</errorMessage>")
*/
}