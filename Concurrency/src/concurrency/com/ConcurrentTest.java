...
public class ConcurrentTest {
    ...
    public void start() {
        ...
        processor.addListValue(new Vector(20,30));
        processor.addListValue(new Vector(20,30));
        processor.addListValue(new Vector(20,30));
    ...
    public void deleteUseless(){
        //for (SummProc proc : listProcs){
        for (int i = 0; i < listProcs.size()-1; i++){
            //System.out.println("** " + proc.getName() + " isAlive - " + proc.isAlive() + ", isSet - " + proc.getVector().isSet());
            //System.out.println("** " + proc.getName() + " isCalculated" + proc.getVector().isCalculate());
            //System.out.println("** " + proc.getName() + " " + proc.getVector().getA() + " " + proc.getVector().getB());
            if ((listProcs.get(i).getVector().isSet()) && (!listProcs.get(i).getVector().isCalculate())) listProcs.get(i).action();
            if (listProcs.get(i).isFree()) {
                System.out.println("** " + listProcs.get(i).getName() + " stoped ");
                listProcs.get(i).interrupt();
                listProcs.remove(i);
            }
        }
    }
            ...
            do {
                ...
                if (vectors.size() > 0){
                    ...
                    if (calculateStart == calculateCount){
                        ...
                        System.out.println("\t\t\t\tend proc " + this.getName());
                        return;                    
                    }
